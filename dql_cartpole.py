...
class Agent():
    ...
    def save(self):
        torch.save(self.Q_eval.state_dict(), 'dql_cartpole.model')
        with open('dql_cartpole.p', 'wb') as pfile:
    ...
    def load(self):
        if os.path.exists('dql_cartpole.model') and os.path.exists('dql_cartpole.p'):
            self.Q_eval.load_state_dict(torch.load('dql_cartpole.model'))
            with open('dql_cartpole.p', 'rb') as pfile:
...
def main(show=False, n_games=20000):
    if show:
        env = gym.make('CartPole-v1', render_mode='human')
    else:
        env = gym.make('CartPole-v1')
    agent = Agent(gamma=0.99, epsilon=1.0, batch_size=256, n_actions=2, eps_end=0.01, eps_dec=1e-6, input_dims=[4], lr=0.003)
    agent.load()
    scores, eps_history = [], []
    try:
